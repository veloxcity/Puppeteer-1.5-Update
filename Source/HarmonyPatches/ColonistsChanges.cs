...
namespace Puppeteer
{
	...
	// RimWorld 1.5: GenSpawn.Spawn signature changed, use TargetMethod
	[HarmonyPatch]
	static class GenSpawn_Spawn_Patch
	{
		static MethodBase TargetMethod()
		{
			// Find the main Spawn method
			return typeof(GenSpawn).GetMethods()
				.Where(m => m.Name == "Spawn" && m.ReturnType == typeof(Thing))
		...
		{
			if (__result is Pawn pawn && pawn.IsColonist)
