...
namespace Puppeteer
{
	...
	static class MainMenuDrawer_Init_Patch
	{
		...
		public static void Postfix()
		{
			if (gameEntered)
				Controller.instance.SetEvent(PuppeteerEvent.GameExited);
			gameEntered = false;
			// VersionInformation.Show(); // Temporarily disabled - may cause input blocking
		}
	}
	...
	static class GizmoGridDrawer_DrawGizmoGrid_Patch
	{
		static Command_Action CreateDeleteResurrectionPortal(ResurrectionPortal portal)
		{
			...
			// RimWorld 1.5: disabled is now protected, order is removed
			if (h > 0)
				cmd.Disable("You have to wait " + hours + " to remove the portal");
			return cmd;
		}
		...
		{
			// RimWorld 1.5: Check if we're on a map (not world view) before accessing Selector
			if (Find.CurrentMap == null || Find.Selector == null) return;
