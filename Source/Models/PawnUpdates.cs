	...
	public class PawnJSON : ThingJSON
	{
		...
		public static PawnJSON Make(Pawn p)
		{
			...
			return new PawnJSON()
			{
				...
				stunned = p.stances != null && p.stances.stunner.Stunned,
				...
				// RimWorld 1.5: AreaRestriction renamed to AreaRestrictionInPawnCurrentMap
				allowedArea = p.playerSettings?.AreaRestrictionInPawnCurrentMap?.Label,
				medCare = p.playerSettings?.medCare.GetLabel(),
				hostility = p.playerSettings?.hostilityResponse.GetLabel(),
				selfTend = p.playerSettings != null && p.playerSettings.selfTend,
				fireAtWill = p.drafter.FireAtWill,
				// RimWorld 1.5: CurrentOutfit renamed to CurrentApparelPolicy
				outfit = p.outfits?.CurrentApparelPolicy?.label,
				drugs = p.drugs?.CurrentPolicy?.label,
				// RimWorld 1.5: CurrentFoodRestriction renamed to CurrentFoodPolicy
				food = p.foodRestriction?.CurrentFoodPolicy?.label,
