	...
	public class ColonistBaseInfo : JSONConvertable<ColonistBaseInfo>
	{
		...
		public class NeedInfo
		{
			...
			public NeedInfo(Need rNeed)
			{
				...
				// RimWorld 1.5: threshPercents is now protected, use reflection or empty array
				treshholds = GetThreshPercents(rNeed);
			}

			private static int[] GetThreshPercents(Need need)
			{
				try
				{
					var field = typeof(Need).GetField("threshPercents", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
					if (field != null)
					{
						var thresholds = field.GetValue(need) as System.Collections.Generic.List<float>;
						if (thresholds != null)
							return thresholds.Select(p => (int)(p * 100)).ToArray();
					}
				}
				catch { }
