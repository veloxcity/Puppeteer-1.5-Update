	...
	public static class GizmosHandler
	{
		...
		*/
		...
		public static List<Command> GetCommands(Pawn pawn, IntVec3 cell, object obj)
		{
			...
			// RimWorld 1.5: Command.order is removed, sort by default order
		...
		public static List<Item> GetActions(object obj, List<Command> commands)
		{
			...
			// RimWorld 1.5: Gizmo.disabled is now protected, use reflection
			bool IsDisabled(Gizmo gizmo)
			{
				try
				{
					var field = typeof(Gizmo).GetField("disabled", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
					if (field != null)
						return (bool)field.GetValue(gizmo);
				}
				catch { }
				return false;
			}
				...
				.Select(cmd =>
				{
					...
					if (cmd is Designator des)
					{
						...
						return new Item
						{
							...
							disabled = IsDisabled(des) ? des.disabledReason : null,
							allowed = Allowed(cmd),
							action = delegate
							{
					...
					return new Item
					{
						...
						disabled = IsDisabled(cmd) ? cmd.disabledReason : null,
						allowed = Allowed(cmd),
