...
namespace Puppeteer
{
	...
	public static class Dialogs
	{
		...
		{
			// RimWorld 1.5: curX and curY are now protected, use GetRect approach
			var curY = GetListingCurY(list);
			var curX = GetListingCurX(list);
			var rect = new Rect(curX, curY, list.ColumnWidth, height > 0f ? height : Text.LineHeight);
			if (Mouse.IsOver(rect))
				SettingsDrawer.currentHelpItem = helpItem;
		}

		// Helper methods to access protected curX/curY via reflection
		private static float GetListingCurY(Listing_Standard list)
		{
			try
			{
				var field = typeof(Listing).GetField("curY", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
				if (field != null)
		...
		private static float GetListingCurX(Listing_Standard list)
		{
			try
			{
				var field = typeof(Listing).GetField("curX", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
				if (field != null)
					return (float)field.GetValue(list);
			}
			catch { }
		...
		public static void Dialog_Checkbox(this Listing_Standard list, string labelId, ref bool forBool)
		{
			...
			// RimWorld 1.5: curX is protected, skip indent manipulation
			var anchor = Text.Anchor;
