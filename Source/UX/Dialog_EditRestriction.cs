...
namespace Puppeteer
{
	public class Dialog_EditRestriction : Window
	{
		...
		public void DoMatcherRow(Rect rect, int i, Matcher matcher)
		{
			...
			rect.x += rect.width + 4f;
			rect.width = 24f;
			// RimWorld 1.5: Use ButtonInvisible + DrawTexture instead of ButtonImage
			GUI.DrawTexture(rect, Assets.DeleteX);
		...
		public List<string> GetMatchingButtons()
		{
			...
			if (lastMatcherText1 != matcher.text)
			{
				...
				// RimWorld 1.5: thingGrid is now private, use public methods instead
				labels.AddRange(
